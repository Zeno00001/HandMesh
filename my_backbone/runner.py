...
class Runner(object):
    ...
    def train(self):
        ...
        for step, data in enumerate(self.train_loader):
            ...
            adjust_learning_rate(
                ...
                epoch=self.epoch,
                step=step,
                len_epoch=len(self.train_loader),
                lr=self.cfg.TRAIN.LR,
                lr_decay=self.cfg.TRAIN.LR_DECAY,
                decay_step=self.cfg.TRAIN.DECAY_STEP,  # [20, 40, 60,], when to use lr decay
            ...
            if self.total_step % 100 == 0:
                ...
                info = {
                    ...
                    'l1_loss': losses.get('joint_img_loss', 0),
                    'epoch': self.epoch,
    ...
    def eval(self):
        ...
        with torch.no_grad():
            ...
            if self.board is not None:
                self.board_scalar('test', self.epoch, **{'2d_auc_loss': auc_2d, 'uve': uve})
                self.board_img('test', self.epoch, data, out, {})
            elif self.args.world_size < 2:
                print( f'uve: {uve}, auc_2d: {auc_2d}')
            self.writer.print_str( f'uve: {uve}, auc_2d: {auc_2d}')  # uve: L2-dist in pixel, area_under...

        return uve  # 2d joint error.mean()
