...
"""
 * @file mobrecon_ds_conf_transformer_conf.py
 * @author chenxingyu (chenxy.sean@gmail.com)
 * @edited clashroyaleisgood @github
 * @brief MobRecon + Transformer model + single image only
 * 
...
class MobRecon_DS_conf_Transformer_Single(nn.Module):
    def __init__(self, cfg):
        ...
        Args:
            cfg : config file
        """
        super(MobRecon_DS_conf_Transformer_Single, self).__init__()
        self.cfg = cfg
    ...
    def forward(self, x):
        ...
        out = {
            ...
            'verts': pred3d,
            'joint_img': pred2d_pt[:, :, :2],  # (B, 21, 2)
            'joint_conf': pred2d_pt[:, :, 2:], # (B, 21, 1)
        }
        if pred_j3d != []:
            # combine list(tensor) to tensor if exists
            out['joints'] = rearrange(pred_j3d, 'H B J D -> H B J D',)  # (H B J D) H for each Head output
        return out
    ...
    def loss(self, **kwargs):
        ...
        ## verts
        verts_pred = rearrange(kwargs['verts_pred'], 'B V D -> B V D')    # (B, 778, 3)
        verts_gt   = rearrange(kwargs['verts_gt'], 'B V D -> B V D')      # (B, 778, 3)
        ## joint
        joint_img_pred  = rearrange(kwargs['joint_img_pred'], 'B J D -> B J D')   # (B, 21, 2)
        joint_img_gt    = rearrange(kwargs['joint_img_gt'], 'B J D -> B J D')     # (B, 21, 2)
        joint_conf_pred = rearrange(kwargs['joint_conf_pred'], 'B J D -> B J D')  # (B, 21, 1)
        ...
        if kwargs.get('joint_3d_pred') is not None:
            Headcounts = kwargs['joint_3d_pred'].shape[0]  # (H B J 3), H for each Head output
            joint_preds = rearrange(kwargs['joint_3d_pred'], 'H B J D -> H B J D')
            joint_gt = rearrange(kwargs['joint_3d_gt'], 'B J D -> B J D')
...
class SequencialReg2DDecode3D(nn.Module):
    ...
    def forward(self, uvc, x, frame_len, feat8x8):
        ...
        EncodingChannelSplit = [156, 100]  # for 2D encodings and temporal encodings
        NewTemporalEncodings = False
        if NewTemporalEncodings == False:
            # uv_embed = uv_encoding(uv, feature_len=self.latent_size // 2)
            serial_embed = self.serial_embed.weight
        else:
            uv_embed = uv_encoding(uv, feature_len=EncodingChannelSplit[0] // 2)
            uv_embed = zero_pad(uv_embed, 256, pad_behind=True)
            serial_embed = t_encoding(frame_len, feature_len=EncodingChannelSplit[1], device=x.device)
        ...
        x, enc_x = self.transformer(x, joint_embedding=self.joint_embed.weight, verts_embedding=self.verts_embed.weight,
                                    serial_embedding=serial_embed, positional_embedding=None,
...
if __name__ == '__main__':
    ...
    model = MobRecon_DS_conf_Transformer_Single(cfg)
    # model.eval()
    # (#, 256, 4, 4), (#, 21, 2)
